    ...
    <div class="d-flex align-center flex-wrap ga-2">
      ...
      <v-tooltip v-if="hpoData.classification?.clinical_group?.primary" location="bottom">
        ...
        <div class="pa-2">
          ...
          <div class="text-caption">
            Score: {{ (hpoData.classification.clinical_group.scores[hpoData.classification.clinical_group.primary] * 100).toFixed(0) }}%
          </div>
        </div>
      </v-tooltip>
      ...
      <v-tooltip v-if="hpoData.classification?.onset_group?.primary" location="bottom">
        ...
        <div class="pa-2">
          ...
          <div class="text-caption">
            ...
            ({{ (hpoData.classification.onset_group.scores[hpoData.classification.onset_group.primary] * 100).toFixed(0) }}%)
          </div>
        </div>
      </v-tooltip>
      ...
      <v-tooltip v-if="hpoData.classification?.syndromic_assessment" location="bottom">
        <template #activator="{ props }">
          <v-chip
            ...
            :variant="hpoData.classification.syndromic_assessment.is_syndromic ? 'tonal' : 'outlined'"
            size="small"
            v-bind="props"
          >
        ...
        <div class="pa-2">
          ...
          <div class="text-caption">
            {{ hpoData.classification.syndromic_assessment.is_syndromic ? 'Syndromic kidney disease' : 'Isolated kidney phenotype' }}
          </div>
          <div v-if="hpoData.classification.syndromic_assessment.extra_renal_categories?.length" class="text-caption mt-1">
            Extra-renal: {{ hpoData.classification.syndromic_assessment.extra_renal_categories.join(', ') }}
          </div>
        </div>
      </v-tooltip>
      ...
      <v-tooltip location="bottom">
        ...
        <div class="pa-2 max-width-300">
          ...
          <div class="text-caption mb-1">
            {{ hpoData.kidney_phenotype_count }} kidney-related phenotypes out of {{ hpoData.phenotype_count }} total
          </div>
        </div>
      </v-tooltip>
...
// Helper functions
const getClinicalGroupColor = (group: string): string => {
...
const formatClinicalGroup = (group: string): string => {
...
const formatOnsetGroup = (group: string): string => {
  const labels: Record<string, string> = {
    adult: 'Adult',
    pediatric: 'Pediatric',

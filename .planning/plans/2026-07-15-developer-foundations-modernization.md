...
- [ ] **Step 3: Add non-mutating checks.** `check` depends on `check-backend check-frontend check-mcp`. Backend check runs `uv lock --check`, `ruff check app/`, `ruff format --check app/`, and `pytest tests/ -v`. Frontend check runs `npm run lint:check`, `npm run format:check`, `npm run build`, and `npm run test:run`. MCP check runs `uv lock --check`, `ruff check .`, `ruff format --check .`, `mypy src/`, `pytest`, then `make -C mcp contract-verify`.

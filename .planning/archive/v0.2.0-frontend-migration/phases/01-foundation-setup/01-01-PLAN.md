...
must_haves:
  truths:
    ...
    - "Prettier format:check covers .ts files alongside .js and .vue"
  artifacts:
...
<objective>
Install TypeScript tooling, create tsconfig project references, rename vite.config to TypeScript, update Prettier to cover .ts files, and create domain type interface stubs for all major entities in the application.

Purpose: Establishes the TypeScript foundation that every subsequent phase depends on. With `allowJs: true` and `checkJs: false`, existing JS files continue working unchanged while new TS files get full type checking. Prettier is updated to format .ts files so all new TypeScript code passes `make ci-frontend` format checks.

Output: Working TypeScript configuration, renamed vite.config.ts, Prettier covering .ts files, domain type interfaces in `src/types/`.
</objective>
...
<task type="auto">
  ...
  <action>
    1. Install TypeScript dependencies (including @tsconfig/node22 explicitly):
       ```bash
       cd frontend
       npm install -D typescript@^5.7.0 vue-tsc@^2.2.0 @vue/tsconfig@^0.8.0 @types/node@^22.0.0 @tsconfig/node22
       ```
       IMPORTANT: @tsconfig/node22 is explicitly installed because tsconfig.node.json extends it. It may or may not be a transitive dependency of @vue/tsconfig — installing it explicitly guarantees it is present.
    ...
    7. Update `frontend/package.json` scripts to include `.ts` in Prettier patterns:
       Change the `format` and `format:check` scripts from:
       ```json
       ...
       To:
       ```json
       "format": "prettier --write \"src/**/*.{js,ts,mjs,vue,css,md}\"",
       "format:check": "prettier --check \"src/**/*.{js,ts,mjs,vue,css,md}\""
       ```
       This ensures that all new TypeScript files created in this plan and subsequent plans are covered by `npm run format:check`, which is called by `make ci-frontend` and GitHub Actions CI.
  </action>
  <verify>
    Run from `frontend/`:
    - `npx vue-tsc --version` outputs version (confirms installation)
    - `node -e "require.resolve('@tsconfig/node22/tsconfig.json')"` exits 0 (confirms @tsconfig/node22 is installed)
    - `cat tsconfig.json | grep references` confirms project references exist
    - `cat tsconfig.app.json | grep '"strict": true'` confirms strict mode
    - `cat tsconfig.app.json | grep '"allowJs": true'` confirms allowJs
    - `cat tsconfig.app.json | grep '"checkJs": false'` confirms checkJs disabled
    - `cat package.json | grep format:check` shows `.ts` in the glob pattern
  </verify>
  <done>
    TypeScript packages installed (including @tsconfig/node22 explicitly), three tsconfig files created with project references, env.d.ts declares Vite globals and Vue module types, strict mode is ON, allowJs is true, checkJs is false. ESLint recognizes .ts files. Prettier format and format:check scripts cover .ts files.
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    ...
    - `cd frontend && npm run format:check` does not report formatting errors in the new .ts files
  </verify>
  <done>
    vite.config.ts is the active config (JS version removed via git mv), five type definition files exist in src/types/ covering all major domain entities (gene, auth, API responses, pipeline), and the project builds and runs successfully. All new .ts files pass Prettier format:check.
  </done>
</task>
...
3. `ls frontend/tsconfig.json frontend/tsconfig.app.json frontend/tsconfig.node.json frontend/env.d.ts` -- all four config files exist
4. `ls frontend/src/types/gene.ts frontend/src/types/auth.ts frontend/src/types/api.ts frontend/src/types/pipeline.ts frontend/src/types/index.ts` -- all five type files exist
5. `ls frontend/vite.config.ts` exists and `ls frontend/vite.config.js` does NOT exist
6. `cd frontend && npm run format:check` passes (no formatting errors, .ts files included)
7. `node -e "require.resolve('@tsconfig/node22/tsconfig.json')" --prefix frontend` confirms @tsconfig/node22 is resolvable
</verification>
...
- Prettier format:check includes .ts files in its glob pattern
</success_criteria>

...
must_haves:
  truths:
    ...
    - "GitHub Actions frontend-ci job runs vitest (local and remote CI are in sync)"
  ...
  artifacts:
    ...
    - path: ".github/workflows/ci.yml"
      provides: "GitHub Actions CI workflow with vitest step in frontend-ci job"
      contains: "vitest"
  ...
  key_links:
    ...
    - from: ".github/workflows/ci.yml"
      ...
      via: "npm run test:run"
      pattern: "vitest|test:run"
---

<objective>
Install and configure Vitest for component testing with a smoke test that mounts a real shadcn-vue Button, update both the Makefile CI pipeline and GitHub Actions workflow to include Vitest, and produce a complete MDI-to-Lucide icon mapping document for all 198 unique icons.

Purpose: Completes the Phase 1 foundation. Vitest proves the test infrastructure works end-to-end (mount Vue component in jsdom with Tailwind classes). The icon audit is a Phase 3 prerequisite -- without it, the icon migration cannot be planned. GitHub Actions CI is updated to match local CI so there is no divergence.

Output: Working Vitest with passing smoke test, updated Makefile and GitHub Actions CI, complete icon audit document.
</objective>
...
<task type="auto">
  <name>Task 1: Install Vitest, create config, write smoke test, update Makefile and GitHub Actions CI</name>
  <files>
    frontend/package.json
    frontend/vitest.config.ts
    frontend/src/test/setup.ts
  ...
  <action>
    ...
    8. Update `.github/workflows/ci.yml` to add a Vitest step to the frontend-ci job. Add the following step AFTER the "Build frontend" step:
       ```yaml
       # Tests - matches npm run test:run
       - name: Run frontend tests (Vitest)
         working-directory: frontend
         run: npx vitest run
       ```
       This keeps GitHub Actions CI in sync with `make ci-frontend`. Without this step, local CI runs Vitest but GitHub Actions CI does not, causing silent divergence.

    9. Verify the full CI pipeline works:
       ```bash
       make ci-frontend
       ```
  </action>
  <verify>
    ...
    - `grep "vitest" .github/workflows/ci.yml` confirms GitHub Actions CI integration
    - `make ci-frontend` exits 0 (full CI pipeline including tests passes)
  </verify>
  <done>
    Vitest is installed and configured with jsdom. A smoke test mounts a real shadcn-vue Button component, verifies rendering, variant handling, and click events. All 3 tests pass. The Makefile ci-frontend target now includes Vitest as Step 4/4, and a standalone make test-frontend target exists. The GitHub Actions CI workflow (.github/workflows/ci.yml) includes a Vitest step in the frontend-ci job, keeping local and remote CI in sync.
  </done>
</task>
...
3. `grep "vitest" .github/workflows/ci.yml` confirms GitHub Actions CI includes Vitest
4. `cat .planning/phases/01-foundation-setup/01-icon-audit.md | head -10` shows audit header with counts
5. `grep -c "mdi-" .planning/phases/01-foundation-setup/01-icon-audit.md` >= 198
6. `ls frontend/vitest.config.ts frontend/src/test/setup.ts frontend/src/components/ui/__tests__/Button.spec.ts` all exist

Overall Phase 1 verification (run after all 3 plans complete):
7. `cd frontend && npm run build` exits 0
8. `cd frontend && npm run dev` starts without errors
9. `ls frontend/tsconfig.json frontend/components.json frontend/vitest.config.ts` all exist
10. `ls frontend/src/types/gene.ts frontend/src/lib/utils.ts frontend/src/components/ui/button/Button.vue` all exist
</verification>
...
- GitHub Actions CI (.github/workflows/ci.yml) frontend-ci job includes Vitest step
</success_criteria>

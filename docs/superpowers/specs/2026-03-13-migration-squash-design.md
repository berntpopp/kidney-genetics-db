...
2. Autogenerate a migration against a blank database (captures tables, columns, indexes, constraints)
...
- `progress_metadata`: Change `JSON` to `JSONB`
...
Autogenerate captures all 21 tables with columns, PKs, FKs, unique constraints, and standard indexes. It also auto-creates `source_status`, `setting_type`, and `setting_category` ENUMs.
...
| key | value | value_type | category | is_sensitive |
|-----|-------|------------|----------|--------------|
| cache.default_ttl | 3600 | number | cache | false |
| cache.max_memory_size | 1000 | number | cache | false |
| cache.cleanup_interval | 3600 | number | cache | false |
| security.jwt_expire_minutes | 30 | number | security | false |
| security.max_login_attempts | 5 | number | security | false |
| security.account_lockout_minutes | 15 | number | security | false |
| security.jwt_secret_key | "***PLACEHOLDER***" | string | security | true |
| pipeline.hgnc_batch_size | 50 | number | pipeline | false |
| pipeline.hgnc_retry_attempts | 3 | number | pipeline | false |
| pipeline.hgnc_cache_enabled | true | boolean | pipeline | false |
| backup.retention_days | 7 | number | backup | false |
| backup.compression_level | 6 | number | backup | false |
| features.auto_update_enabled | true | boolean | features | false |
| logging.log_retention_days | 30 | number | logging | false |

**static_sources** (2 rows via `INSERT ... ON CONFLICT DO NOTHING`):

| source_name | source_type | display_name |
|-------------|-------------|--------------|
| DiagnosticPanels | hybrid | Diagnostic Panels |
| Literature | hybrid | Literature Evidence |
...
Remove all 18 `.py` files from `backend/alembic/versions/`. Preserved in git tag + `.planning/archive/alembic_v1_migrations/`.

...
def main():
    ...
    parser.add_argument(
        "--pmid",
        type=str,
        help="Process only a specific PMID"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        ...
        if args.pmid:
            ...
            try:
                ...
                logger.info(f"Successfully processed PMID {args.pmid}: {result.total_unique_genes} genes")
            ...
            # List output files
            output_dir = scraper.output_dir / scraper.output_dir.parent.name

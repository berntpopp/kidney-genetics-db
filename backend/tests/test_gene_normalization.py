...
class TestGeneNormalization:
    ...
    def test_normalize_gene_for_database_existing_gene(
        self, mock_get_client, mock_get_gene, mock_db, mock_existing_gene
    ):
        ...
        result = normalize_gene_for_database(
            db=mock_db, gene_text="PKD1", source_name="Test", original_data={"test": True}
        )
    ...
    @patch("app.core.gene_normalization.gene_crud.get_gene_by_symbol")
    @patch("app.core.gene_normalization.get_hgnc_client")
    def test_normalize_gene_for_database_hgnc_success(
        self, mock_get_client, mock_get_gene, mock_db
    ):
    ...
    def test_normalize_gene_for_database_hgnc_failure(
        self, mock_create_staging, mock_get_client, mock_get_gene, mock_db, mock_staging_record
    ):
        ...
        result = normalize_gene_for_database(
            db=mock_db, gene_text="INVALID_GENE", source_name="Test", original_data={"test": True}
        )
    ...
    def test_normalize_gene_for_database_invalid_symbol(self, mock_create_staging, mock_db):
        ...
        result = normalize_gene_for_database(
            db=mock_db, gene_text="INVALID123", source_name="Test", original_data={"test": True}
        )
    ...
    def test_normalize_gene_for_database_empty_input(self, mock_db):
        ...
        with patch("app.core.gene_normalization._create_staging_record") as mock_create_staging:
            ...
            result = normalize_gene_for_database(db=mock_db, gene_text="", source_name="Test")
    ...
    def test_normalize_genes_batch_empty_input(self, mock_get_client, mock_get_gene, mock_db):
        ...
        result = normalize_genes_batch(db=mock_db, gene_texts=[], source_name="Test")
    ...
    @patch("app.core.gene_normalization.gene_crud.get_gene_by_symbol")
    @patch("app.core.gene_normalization.get_hgnc_client")
    def test_normalize_genes_batch_mixed_results(
        self, mock_get_client, mock_get_gene, mock_db, mock_existing_gene
    ):
        ...
        mock_hgnc_client.standardize_symbols_parallel.return_value = {
            "NEW_GENE": {"approved_symbol": "NEW_GENE", "hgnc_id": "HGNC:12345"},
            "INVALID_GENE": {
                "approved_symbol": "INVALID_GENE",
        ...
        with patch("app.core.gene_normalization._create_staging_record") as mock_create_staging:
            ...
            result = normalize_genes_batch(db=mock_db, gene_texts=gene_texts, source_name="Test")
    ...
    @patch("app.core.gene_normalization.gene_crud.get_gene_by_symbol")
    @patch("app.core.gene_normalization.get_hgnc_client")
    def test_normalize_genes_batch_all_existing(
        self, mock_get_client, mock_get_gene, mock_db, mock_existing_gene
    ):
    ...
    @patch("app.core.gene_normalization.gene_crud.get_gene_by_symbol")
    @patch("app.core.gene_normalization.get_hgnc_client")
    def test_normalize_genes_batch_with_original_data(
        self, mock_get_client, mock_get_gene, mock_db
    ):
        """Test batch normalization with original data contexts."""
        gene_texts = ["PKD1", "PKD2"]
        original_data_list = [{"pmid": "12345", "mentions": 5}, {"pmid": "67890", "mentions": 3}]
    ...
    @patch("app.core.gene_normalization.get_hgnc_client")
    def test_get_normalization_stats(
        self, mock_get_client, mock_staging_stats, mock_count_hgnc, mock_count_total, mock_db
    ):
        ...
        expected = {
            ...
            "staging_records": {"pending": 30, "approved": 15, "rejected": 5, "total": 50},
            "hgnc_cache_info": {"symbol_to_hgnc_id": {"hits": 100, "misses": 10}},
        }
        assert result == expected
...
class TestIntegration:
    ...
    def test_end_to_end_normalization_workflow(self, mock_get_client, mock_get_gene, mock_db):
        ...
        mock_hgnc_client = Mock()
        mock_hgnc_client.standardize_symbols_parallel.return_value = {
            "NEW_GENE": {"approved_symbol": "NEW_GENE_CORRECTED", "hgnc_id": "HGNC:12345"}
        }
        mock_get_client.return_value = mock_hgnc_client
        ...
        with patch("app.core.gene_normalization._create_staging_record") as mock_create_staging:
            ...
            # Process batch
            result = normalize_genes_batch(
                db=mock_db, gene_texts=gene_texts, source_name="TestSource"
            )
    ...
    def test_performance_characteristics(self, mock_db):
        ...
        with patch("app.core.gene_normalization.gene_crud.get_gene_by_symbol") as mock_get_gene:
            with patch("app.core.gene_normalization.get_hgnc_client") as mock_get_client:
                ...
                # Process batch
                result = normalize_genes_batch(
                    db=mock_db, gene_texts=large_gene_list, source_name="TestSource"
                )

...
class TestPubTatorNormalization:
    ...
    @patch('app.pipeline.sources.pubtator.normalize_genes_batch')
    @patch('app.pipeline.sources.pubtator.gene_crud.create_or_update_gene')
    def test_all_genes_require_staging(self, mock_create_gene, mock_normalize_batch, client, mock_db):
        ...
        with patch.object(client, '_fetch_pubtator_data') as mock_fetch:
            mock_fetch.return_value = {gene: {"mentions": 5, "pmids": ["123"]} for gene in gene_list}
    ...
    @patch('app.pipeline.sources.pubtator.normalize_genes_batch')
    @patch('app.pipeline.sources.pubtator.gene_crud.create_or_update_gene')
    def test_gene_creation_failure_handling(self, mock_create_gene, mock_normalize_batch, client, mock_db):
...
class TestNormalizationIntegration:
    ...
    def test_pipeline_run_with_normalization(self, mock_db):
        ...
        with patch('app.pipeline.sources.pubtator.PubTatorClient') as mock_client_class:
            with patch('app.pipeline.sources.pubtator.gene_crud.create_pipeline_run') as mock_create_run:
                with patch('app.pipeline.sources.pubtator.gene_crud.update_pipeline_run') as mock_update_run:

...
def upgrade():
    ...
    op.create_index(
        "idx_annotations_gin", "gene_annotations", ["annotations"], postgresql_using="gin"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "panelapp_panels", postgresql.ARRAY(sa.Text()), server_default="{}", nullable=False
        ),
        sa.Column(
            "literature_refs", postgresql.ARRAY(sa.Text()), server_default="{}", nullable=False
        ),
        sa.Column(
            "diagnostic_panels", postgresql.ARRAY(sa.Text()), server_default="{}", nullable=False
        ),
        sa.Column("hpo_terms", postgresql.ARRAY(sa.Text()), server_default="{}", nullable=False),
        sa.Column(
            "pubtator_pmids", postgresql.ARRAY(sa.Text()), server_default="{}", nullable=False
        ),
    ...
    op.create_index(
        "idx_gene_normalization_staging_original_text",
        "gene_normalization_staging",
        ["original_text"],
    )
    op.create_index(
        "idx_gene_normalization_staging_source_name", "gene_normalization_staging", ["source_name"]
    )
    op.create_index(
        "idx_gene_normalization_staging_status", "gene_normalization_staging", ["status"]
    )
    op.create_index(
        "idx_gene_normalization_staging_created_at", "gene_normalization_staging", ["created_at"]
    )
    op.create_index(
        "idx_gene_normalization_staging_priority_score",
        "gene_normalization_staging",
        ["priority_score"],
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["staging_id"], ["gene_normalization_staging.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "idx_gene_normalization_log_original_text", "gene_normalization_log", ["original_text"]
    )
    op.create_index(
        "idx_gene_normalization_log_source_name", "gene_normalization_log", ["source_name"]
    )  # ADDED
    ...
    op.create_index(
        "idx_gene_normalization_log_approved_symbol", "gene_normalization_log", ["approved_symbol"]
    )
    op.create_index(
        "idx_gene_normalization_log_created_at", "gene_normalization_log", ["created_at"]
    )
    ...
    op.create_index(
        "idx_static_evidence_uploads_source_id", "static_evidence_uploads", ["source_id"]
    )
    ...
    op.create_index(
        "idx_system_logs_timestamp_level", "system_logs", [sa.text("timestamp DESC"), "level"]
    )

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("idle", "running", "completed", "failed", "paused", name="source_status"),
            nullable=False,
            server_default="idle",
        ),
    ...
    op.create_index(
        "ix_gene_normalization_staging_original_text",
        "gene_normalization_staging",
        ["original_text"],
    )
    op.create_index(
        "ix_gene_normalization_staging_source_name", "gene_normalization_staging", ["source_name"]
    )
    op.create_index(
        "ix_gene_normalization_staging_status", "gene_normalization_staging", ["status"]
    )
    op.create_index(
        "ix_gene_normalization_staging_created_at", "gene_normalization_staging", ["created_at"]
    )
    op.create_index(
        "ix_gene_normalization_staging_priority_score",
        "gene_normalization_staging",
        ["priority_score"],
    )
    ...
    op.create_index(
        "ix_gene_normalization_log_original_text", "gene_normalization_log", ["original_text"]
    )
    op.create_index(
        "ix_gene_normalization_log_source_name", "gene_normalization_log", ["source_name"]
    )
    op.create_index("ix_gene_normalization_log_success", "gene_normalization_log", ["success"])
    op.create_index(
        "ix_gene_normalization_log_approved_symbol", "gene_normalization_log", ["approved_symbol"]
    )
    op.create_index("ix_gene_normalization_log_hgnc_id", "gene_normalization_log", ["hgnc_id"])
    op.create_index(
        "ix_gene_normalization_log_created_at", "gene_normalization_log", ["created_at"]
    )

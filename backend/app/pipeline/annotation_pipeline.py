...
class AnnotationPipeline:
    ...
    async def _update_sources_parallel(
        ...
        semaphore = asyncio.Semaphore(3)  # Allow 3 concurrent sources (pool_size=15, each source uses ~2-3 connections)
    ...
    async def _update_source_with_session(
        ...
        Uses the provided ``source_db`` session instead of ``self.db``,
        ensuring full isolation from other concurrent source updates.

    ...
    async def fetch_with_cache(
        ...
        try:
            ...
            if self.cache_service and data is not None:
                try:
                    await self.cache_service.set(full_key, data, namespace="default", ttl=effective_ttl)
                    ...
                    for item in batch:
                        if item in batch_data:
                            ...
                            if self.cache_service:
                                cache_key = f"{self.namespace}:{cache_key_func(item)}"
                                try:
                                    await self.cache_service.set(cache_key, data, namespace="default", ttl=effective_ttl)
                                except Exception as e:
                                    logger.sync_warning(

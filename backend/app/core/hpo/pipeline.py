...
class HPOPipeline:
    ...
    async def process_kidney_phenotypes(
        ...
        gene_evidence = {}
        disease_cache = {}  # Cache disease details to avoid duplicate fetches
        ...
        for gene_data in gene_evidence.values():
            gene_data["hpo_terms"] = sorted(list(gene_data["hpo_terms"]))
            gene_data["diseases"] = sorted(list(gene_data["diseases"]))
            gene_data["inheritance_patterns"] = sorted(list(gene_data["inheritance_patterns"]))

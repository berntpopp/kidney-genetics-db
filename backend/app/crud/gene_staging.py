...
class GeneNormalizationStagingCRUD:
    ...
    def create_staging_record(self,
                            db: Session,
                            original_text: str,
                            source_name: str,
                            normalization_log: dict[str, Any],
                            original_data: dict[str, Any] | None = None) -> GeneNormalizationStaging:
        """Create a new staging record for manual review"""
        
        # Calculate priority score based on various factors
        priority_score = self._calculate_priority_score(original_text, source_name, normalization_log)
    ...
    def get_pending_reviews(self, 
                          db: Session, 
                          limit: int = 50,
                          source_filter: str | None = None) -> list[GeneNormalizationStaging]:
        ...
        return query.order_by(
            desc(GeneNormalizationStaging.priority_score),
            GeneNormalizationStaging.created_at
        ).limit(limit).all()
        
    def approve_staging_record(self,
                             db: Session,
                             staging_id: int,
                             approved_symbol: str,
                             hgnc_id: str,
                             ...
                             reviewer: str = "system",
                             notes: str | None = None) -> GeneNormalizationStaging:
        """Approve a staging record with manual corrections"""
        
        staging_record = db.query(GeneNormalizationStaging).filter(
            GeneNormalizationStaging.id == staging_id
        ).first()
    ...
    def reject_staging_record(self,
                            db: Session,
                            staging_id: int,
                            reviewer: str = "system",
                            notes: str | None = None) -> GeneNormalizationStaging:
    ...
    def get_staging_stats(self, db: Session) -> dict[str, Any]:
        ...
        total_pending = db.query(GeneNormalizationStaging).filter(
            GeneNormalizationStaging.status == "pending_review"
        ).count()
        
        total_approved = db.query(GeneNormalizationStaging).filter(
            GeneNormalizationStaging.status == "approved"
        ).count()
        
        total_rejected = db.query(GeneNormalizationStaging).filter(
            GeneNormalizationStaging.status == "rejected"
        ).count()
        ...
        for (source,) in sources:
            source_pending = db.query(GeneNormalizationStaging).filter(
                and_(
    ...
    def _calculate_priority_score(self, 
                                original_text: str, 
                                source_name: str, 
                                normalization_log: dict[str, Any]) -> int:
    ...
    def _requires_expert_review(self, original_text: str, source_name: str) -> bool:
        ...
        important_genes = [
            'PKD1', 'PKD2', 'COL4A1', 'COL4A3', 'COL4A4', 'COL4A5',
            'NPHS1', 'NPHS2', 'UMOD', 'HNF1B', 'ATP6V1B1'
        ]
...
class GeneNormalizationLogCRUD:
    ...
    def create_log_entry(self,
                        ...
                        processing_time_ms: int | None = None) -> GeneNormalizationLog:
    ...
    def get_normalization_stats(self, db: Session) -> dict[str, Any]:
        ...
        successful_attempts = db.query(GeneNormalizationLog).filter(
            GeneNormalizationLog.success == True
        ).count()
        ...
        for (source,) in sources:
            source_total = db.query(GeneNormalizationLog).filter(
                GeneNormalizationLog.source_name == source
            ).count()
            source_success = db.query(GeneNormalizationLog).filter(
                and_(
                    GeneNormalizationLog.source_name == source,
                    GeneNormalizationLog.success == True
                )
